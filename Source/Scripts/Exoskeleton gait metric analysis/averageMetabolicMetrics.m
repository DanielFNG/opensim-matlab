...
for nmetric = 1:length(joint_names)
            ...
            for subject = subjects
                for foot = feet
                    data = MetricsData.(['Subject' num2str(subject)]).AvgJointPowers. ...
                        (joint_names{nmetric}){foot, context, assistance};
    ...
    % Create the metric.
    Metrics.(joint_names{nmetric}) = metric(joint_names{nmetric}, ...
