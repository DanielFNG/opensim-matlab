...
void writeJacobian(std::ofstream& file_name,
				   double time, 
...
int main(int argc, const char * argv[])
{
	...
	std::string INTERNAL_FORCE = JSF_RESULTS + "/internal_force.txt";
	...
	try {
		...
		for (int i=0; i < 100; i++)
		{
			// Get time/dump the first entry of each row of each file. These 
			// are the same so this isn't actually changing time. 
			states_file >> time;
			...
			*/
			...
			writeJacobian(leftAPOJacobian_file, time, leftAPOJacobian);
			writeJacobian(leftAPOJacobian_file, time, rightAPOJacobian);
			
			// Write the residual forces and internal forces (almost identical 
			// to net joint torques but with a slighty discrepancy, a.k.a 
			// residual forces) to file.
			Vector residualForce, internalForce; 
			residualForce = gravityTorques - inertiaTorques + dynamics 
							- coriolisTorques + rightGRFTorques 
							+ leftGRFTorques;
			internalForce = inertiaTorques - gravityTorques + coriolisTorques
							- rightGRFTorques - leftGRFTorques;
			writeVector(residualForce_file, time, residualForce);
			writeVector(internalForce_file, time, internalForce);
